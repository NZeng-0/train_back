...
//    @PreAuthorize("@ss.hasPermi('train:seat:query')")
    ...
    @GetMapping("byTrain")
    public AjaxResult getInfoByTrainAndNumber(
            @RequestParam("id") String id,
            @RequestParam("type") String type,
            @RequestParam("number") String number) {

        return success(seatService.selectSeatByTrainAndType(id, type, number));
    }

    @GetMapping("cancel")
    public AjaxResult cancelSeatTicked(@RequestParam("id") String id) {
        seatService.cancelSeatTicket(id);
        return success();
    }
}
